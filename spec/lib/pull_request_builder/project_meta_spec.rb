...
RSpec.describe ProjectMeta do
  ...
  describe '.to_xml' do
    it { expect(described_class.new('foo', 'bar', repositories_to_build).to_xml).to be_a(String) }
    it { expect(described_class.new('foo', 'bar', repositories_to_build).to_xml).to have_tag('project') }
    it { expect(described_class.new('foo', 'bar', repositories_to_build).to_xml).to have_tag('repository', count: 2) }
  end
end
